...
public sealed class ControlPanelScreenshotTests
{
    ...
    private static void SaveDump(string fileNameStem, string dump)
    {
        ...
        File.WriteAllText(Path.Combine(ImagesDirectory, $"{fileNameStem}.txt"), dump);
    }
    ...
    public async Task ControlPanelMode_K8055_WithLiveInputData_IsCaptured()
    {
        ...
        TuiTestRunner.RunWithLoop(
            ...
            parts =>
            {
                ...
                var updated = TuiTestRunner.WaitUntilOnLoop(() => parts.IndicatorLabels["analogIn1"].Text == "42", WaitTimeout);
                ...
                {
                    Directory.CreateDirectory(ImagesDirectory);
                    TuiScreenshot.Save(Path.Combine(ImagesDirectory, "tui-control-panel-k8055.png"));
                    return true;
                });
            });
        ...
        StringAssert.Contains(dump, "Digital Out");
        StringAssert.Contains(dump, "0x05");
    ...
    public void ControlPanelMode_Busylight_IsCaptured()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                Directory.CreateDirectory(ImagesDirectory);
                TuiScreenshot.Save(Path.Combine(ImagesDirectory, "tui-control-panel-busylight.png"));
            }
            finally
            {
        ...
        StringAssert.Contains(dump, "Color");
        StringAssert.Contains(dump, "Apply");
    }
    ...
    public async Task ControlPanelMode_ScpiKorad_WithQueryReply_IsCaptured()
    {
        ...
        TuiTestRunner.RunWithLoop(
            ...
            parts =>
            {
                ...
                surface.InvokeAsync("vsetQuery", null).GetAwaiter().GetResult();
                transport.PushIncomingAsync("05.00"u8.ToArray()).GetAwaiter().GetResult();

                var updated = TuiTestRunner.WaitUntilOnLoop(() => parts.IndicatorLabels["vsetQuery.reply"].Text == "05.00", WaitTimeout);
                ...
                {
                    Directory.CreateDirectory(ImagesDirectory);
                    TuiScreenshot.Save(Path.Combine(ImagesDirectory, "tui-control-panel-scpi.png"));
                    return true;
                });
            });
        ...
        StringAssert.Contains(dump, "Set Voltage");
        StringAssert.Contains(dump, "05.00");
        ...
        Assert.IsTrue(definition.Sections.Any(s => s.Label == "Custom Command"));
    }
}
