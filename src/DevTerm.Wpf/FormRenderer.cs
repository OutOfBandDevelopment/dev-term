...
internal static class FormRenderer
{
            ...
            case IndicatorControl indicator:
                {
                    ...
                    {
                        text.SetResourceReference(TextBlock.ForegroundProperty, WarningBrushKey);
                    }
    ...
    private static FrameworkElement BuildTextBox(WpfFormParts parts, WpfFormRow row, UiControl control)
    {
        ...
        var error = new TextBlock { TextWrapping = TextWrapping.Wrap, Visibility = Visibility.Collapsed, Margin = new Thickness(0, 2, 0, 0) };
        error.SetResourceReference(TextBlock.ForegroundProperty, WarningBrushKey);
