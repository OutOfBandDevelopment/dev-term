...
- **Recognizing that a reply *is* renderable content in the first place** (vs. plain text/hex) — either from a declared per-command hint (a device profile's own command definition knows its query returns a bitmap or an HPGL stream) or by sniffing known signatures (HPGL mnemonics, PostScript's `%!` header, PCL escape sequences, common image magic bytes) — see the new [stream content detection](proposals/stream-content-detection.md) proposal, which also covers TUI's graphics-free fallback (auto-save the raw capture under a `{device}_{timestamp}.{ext}` name) vs. WPF's native-image-format live preview.
- **Stateful presenter lifetime vs. DI registration.** A buffering presenter (like ASCII's line accumulator) holds state across calls, but presenters are currently registered as DI singletons (see [plugin-model.md](plugin-model.md)) — fine as long as the app only ever has one session/pipeline alive at a time (true today), but wrong once concurrent sessions can share a process (a future TUI/GUI): they'd interleave each other's partial lines/frames through the same instance. Not yet resolved — likely needs presenters (at least stateful ones) to be created per-session rather than resolved once at startup.
