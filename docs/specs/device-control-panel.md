...
| `ButtonControl` with `ColorPickerTargetCommandId` | `Button` that opens a nested RGB/HSV color-picker `Dialog` | `Button` that opens `ColorPickerWindow` | Sends the picked color as `"{r},{g},{b}"` to the *target* command id, not the button's own id. The picker opens on the color last picked for that button (`LastPickedColors`, keyed by the button's control id), including after closing and reopening the panel. It starts on white until a color has been picked, and it's kept only for the life of the process |
