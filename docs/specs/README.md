...
- [Manifest Editor](manifest-editor.md) — `ManifestEditorMode` (TUI) / `ManifestEditorWindow` (WPF)
  over the shared `ManifestEditorViewModel`: create, open, edit and save a device manifest, its forms
  generated by the forms engine, with a live preview of its panel. **Device > Edit Device Manifest...**
  in both front ends.
